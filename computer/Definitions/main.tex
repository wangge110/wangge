...
After training, DLPCM generates the fused keypoint set $K_{\text{final}}$, which is then used as structural observations for the subsequent 3D reconstruction stage. Figure~\ref{fig:dlpcm_fusion} illustrates the masked completion-and-fusion mechanism: visible keypoints predicted by the SMAL-Pig head are retained, while occluded keypoints are imputed from the discrete latent codebook and merged via the visibility mask.
