...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_smal_pig_network.png}
\caption{SMAL-Pig network architecture for single-view 3D reconstruction. The network consists of a 2D keypoint detection branch (GK-HourGlass), a DLPCM module for occluded keypoint completion, and a 3D pose regression module. The SMAL-Pig parametric model provides a unified representation of pig shape and pose, enabling stable 3D mesh recovery from 2D observations.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_occlusion_framework.png}
\caption{Occlusion-robust single-view 3D reconstruction framework. The framework integrates two complementary modules: (1) DLPCM for completing occluded 2D keypoints using discrete latent pose representations, and (2) SC for regularizing joint angles in the pose parameter space. The combination improves both 2D projection consistency and 3D pose plausibility under occlusion.}
\label{fig:occlusion_framework}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_dlpcm_fusion.png}
\caption{Mask-based occluded keypoint completion and fusion mechanism. DLPCM retains visible keypoints (shown in green) predicted by the SMAL-Pig keypoint head and completes only occluded keypoints (shown in red) using the discrete latent codebook. The visibility mask ($M_v$) controls which keypoints are retained versus completed, ensuring that reliable image-based predictions are preserved while missing keypoints are inferred from structural relationships.}
...
\begin{itemize}
  \item \textbf{Stifle (knee):} flexion range 0°--120°; typical gait range 20°--80°.
  \item \textbf{Elbow:} flexion range 0°--110°; typical gait range 15°--70°.
  \item \textbf{Hip:} flexion range 0°--150°; typical gait range 30°--90°.
  \item \textbf{Shoulder:} flexion range 0°--140°; typical gait range 20°--100°.
\end{itemize}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Joint} & \textbf{Threshold $a_j^{\max}$ (rad)} & \textbf{Equivalent angle (°)} & \textbf{Literature range (°)} & \textbf{Validation} \\
\midrule
Stifle (knee) & 0.90 & 51.6 & 0--120 & ✓ within range \\
Elbow & 0.80 & 45.8 & 0--110 & ✓ within range \\
Hip & 1.20 & 68.7 & 0--150 & ✓ within range \\
Shoulder & 1.00 & 57.3 & 0--140 & ✓ within range \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_dataset.png}
\caption{Dataset examples showing diverse pig poses and occlusion types. The dataset includes self-occlusion (pig body parts occluding each other), inter-animal occlusion (multiple pigs occluding each other), and pen-rail occlusion (infrastructure occluding pig body parts). Images are collected from real pig-house environments with varying lighting conditions.}
\label{fig:dataset}
\end{figure}
...
All 3D experiments use the same data splits, preprocessing, backbone, optimizer, schedule, and base loss weights under a BARC-style weak supervision protocol. Inputs are resized to 256 × 256. We train with RMSprop for 220 epochs with an initial learning rate of $5 \times 10^{-4}$, batch size 12, zero momentum, and no weight decay. The learning rate decays by 0.1 at epochs 150, 175, and 200. Thus, performance differences primarily reflect the effects of DLPCM and SC.
...
\toprule
\textbf{Method} & \textbf{2D keypoint source} & \textbf{SC} & \textbf{PCK@0.1 / \%} & \textbf{IoU / \%} & \textbf{JRVR / \%} \\
\midrule
Baseline & SMAL-Pig keypoint head for all & ✗ & 61.85 & 64.04 & 69.74 \\
+DLPCM & SMAL-Pig visible + DLPCM occluded & ✗ & 66.73 & 67.90 & 24.34 \\
+SC & SMAL-Pig keypoint head for all & ✓ & 64.79 & 66.32 & 12.50 \\
+DLPCM+SC & SMAL-Pig visible + DLPCM occluded & ✓ & 64.91 & 66.05 & 9.38 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Qualitative visualization of 3D pig reconstruction results under different occlusion scenarios. Each row shows: (a) input image with occlusion, (b) predicted 2D keypoints overlaid on the image, (c) projected 3D mesh, and (d) rendered 3D mesh from multiple viewpoints. The results demonstrate that the proposed method produces anatomically plausible pig poses even under severe occlusion, with smooth limb configurations and realistic joint angles. In contrast, baseline methods (without DLPCM and SC) often exhibit reverse bending, excessive rotation, or local twisting in occluded regions.}
\label{fig:qualitative}
\end{figure}

Figure~\ref{fig:qualitative} shows qualitative reconstruction results on representative test images. The visualizations include the input image, predicted 2D keypoints, projected mesh, and rendered 3D mesh. The results demonstrate that our method produces anatomically plausible pig poses even under severe occlusion, with smooth limb configurations and realistic joint angles. In contrast, baseline methods often exhibit reverse bending, excessive rotation, or local twisting in occluded regions.
